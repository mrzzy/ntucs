...
void sortStack(Stack *s)
{
	/* add your code here */
	Stack buffer;
	memset(&buffer, 0, sizeof(Stack));
	
	// copy elements into buffer stack
	while(s->ll.size > 0) {
		push(&buffer, pop(s));
	}

	while(buffer.ll.size > 0) {
		// pop element off the buffer stack 
		int x = pop(&buffer);
		// pop elements in input stack that break sorted order
		// push the back into the buffer stack
		while (s->ll.size > 0 && peek(s) < x) {
			push(&buffer, pop(s));
		}
		// insert popped into input stack
		push(s, x);
	}
}
