...
{
  /* add your code here */
  // push first k elements off queue into stack
  Stack s;
  ...
  for (int i = 0; i < k; i++) {
	...
	// pop k elements off the stack into queue
	// since stack is FIFO, elements will be popped in reverse order
  for (int i = 0; i < k; i++) {
		enqueue(q, pop(&s));
	}

	// rotate queue by length - k positions to restore original order of lements
