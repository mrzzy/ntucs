...
int moveMaxToFront(ListNode **ptrHead)
{
	// traverse linked list and locate maxNode
	ListNode* previous = NULL;
	ListNode* maxNode = NULL;
	ListNode* prevMaxNode = NULL;
	ListNode* n = *ptrHead;
	while(n != NULL) {
		// update maxNode if we see a node with a larger item
		if(!maxNode || maxNode->item < n->item) {
			maxNode = n;
			prevMaxNode = previous;
		}
		previous = n;
		n = n->next;
	}
	...
	if(!maxNode) return 0;
	// check if maxNode already first node
	if(*ptrHead == maxNode) return 0;
	
	// unlink maxNode from previous node
	if(prevMaxNode) {
		prevMaxNode->next = maxNode->next;
	}
	// link maxNode as first node
