...
void removeDuplicatesSortedLL(LinkedList *ll)
{
	...
	while (n != NULL) {
		...
		if(!lastUnique) {
			lastUnique = n;
		} else if (n->item == lastUnique->item) {
			// duplicate: remove
			free(n);
		} else {
			// unlink duplicate elements from linked list
			lastUnique->next = next;
			lastUnique = NULL;
		}
		n = next;
	}
	// last unique in linked list ends list
