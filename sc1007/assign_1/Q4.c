...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	// move first half of the list into front list
	// include extra element in first half for odd no. of elements
	int halfSize = ll->size / 2  + ((ll->size % 2 == 0) ? 0 : 1);

	for (int i = 0; i < halfSize; i++) {
		ListNode *move = n;
		if(i == 0) {
			// first element should be in front list
			resultFrontList->head = move;
			frontTail = move;
		} else {
			frontTail->next = move;
		}
	...
	// move back half of the list into back list
