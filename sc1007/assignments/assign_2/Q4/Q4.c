...
double postfixEvaluation(char* postfix)
{
    ...
    for (int i = 0; i < strnlen(postfix, SIZE); i++) {
        double right = 0;
        double left = 0;
        switch (postfix[i]) {
            case '+':
            ...
            case '*':
            case '/':
                // stack reverses operands
                right = pop(&operands);
                left = pop(&operands);
                // evaluate operands with operator
                switch (postfix[i]) {
                    case '+':
                ...
                break;
            default:
                // assume operand digit: push value of digit to stack.
                push(&operands, postfix[i] - '0');
        }
    }

    // obtain result from top of stack
    return pop(&operands);
}
