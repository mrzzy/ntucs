...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    // link with hash table slot
    HashTableNode* slot = &Q1Ptr->Table[Hash(key, Q1Ptr->hSize)];
    if(slot->size <= 0) {
        slot->head = node;
    } else {
        slot->head->pre = node;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    if(!node) {
        // failure: key not stored in table
        return 0;
    }

    // unlink from slot head
    HashTableNode* slot = &Q1Ptr->Table[Hash(key, Q1Ptr->hSize)];
    if(slot->head == node) {
        slot->head = node->next;
    }
    ...
    slot->size --;
    Q1Ptr->nSize --;
    // success: removed node from the hash table
    return 1;
}
