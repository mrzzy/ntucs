...
{
	/* add your code here */
	// no elements can be insert if ll1 has no elements
	if(ll1->size <= 0) return;
	for(ListNode *n = ll1->head; n != NULL && ll2->size > 0; n = n->next) {
		// pop first element from ll2 for insertion
		ListNode* insert = ll2->head;
		ll2->head = ll2->head->next;
		ll2->size --;

		// insert first element off ll2 into into ll1
		insert->next = n->next;
		n->next = insert;
		n = insert;
		ll1->size ++;
